...
export async function insertDoctor(DoctorData: InsertDoctor): Promise<Pick<Doctor, 'id'>> {
  ...
  const query = `
    INSERT INTO Doctors (first_name, last_name, email, password_hash, phone_number, speciality, crm, role, birth_date, cpf)
...
export async function selectAllDoctors(): Promise<FullDoctor[]> {
  ...
  try {
    const { rows } = await pool.query<DoctorRow>(query);
    ...
    throw new AppError({
      statusCode: 500,
      errorMessages: ['Database internal error while fetching all Doctors'],
      originalError: isProd ? undefined : (error as Error),
    });
  }
}
